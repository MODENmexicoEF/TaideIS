    ...
    {
        var pmId = ObtenerIdDesdeToken(); // Recuperar desde JWT
        if (pmId == null)
            return Unauthorized();

        // Aquí puedes validar si el PM realmente está asignado a este paciente (opcional)
        // Si no lo deseas validar, puedes omitir este bloque
        var existePaciente = await _context.Pacientes.AnyAsync(p => p.ID == pacienteId);
        if (!existePaciente)
            return NotFound(new { Message = "Paciente no encontrado." });

        var reportes = await _context.ReportesMedicos
            .Where(r => r.PacienteID == pacienteId && r.PM_ID == pmId)
